...
class FeedbackService {
    ...
    static async getFeedback(filters = {}) {
        const { page = 1, limit = 10, min_rating, max_rating, search, branchId, rating, startDate, endDate, } = filters;
    ...
    `);
        ...
        const distributionRequest = database_1.pool.request();
        if (branchId) {
    ...
    static async getFeedbackById(feedbackId) {
        const result = await database_1.pool.request().input("feedbackId", feedbackId).query(`
      SELECT 
        cf.id,
        cf.order_id,
        cf.user_id,
    ...
    `);
        if (result.recordset.length === 0) {
            throw new error_middleware_1.ApiError(404, "Feedback not found");
        }
    ...
    static async updateFeedbackStatus(feedbackId, isPublished) {
    ...
    static async submitCustomerFeedback(feedbackData) {
        const { branch_id, customer_name, phone_number, email, overall_rating, reception_rating, service_speed_rating, quality_rating, cleanliness_rating, catering_rating, opinion, } = feedbackData;
        ...
        if (overall_rating < 1 || overall_rating > 4) {
            throw new error_middleware_1.ApiError(400, "Rating must be between 1 and 4");
        }
