...
class CategoriesService {
    ...
    static async getCategoryById(id) {
        const result = await database_1.pool.request().input("id", id).query(`
      SELECT 
        id, name_ar, name_en,
        description_ar, description_en,
        image_url, display_order, is_active,
    ...
    static async createCategory(data) {
        const { name_ar, name_en, description_ar, description_en, image_url, display_order, is_active, } = data;
    ...
    static async updateCategory(id, data) {
        ...
        const existsCheck = await database_1.pool
            .request()
            .input("id", id)
    ...
    static async deleteCategory(id) {
        ...
        const productsCheck = await database_1.pool
            .request()
            .input("id", id)
            .query("SELECT COUNT(*) as count FROM products WHERE category_id = @id");
        const productCount = productsCheck.recordset[0].count;
        if (productCount > 0) {
            throw new error_middleware_1.ApiError(400, `Cannot delete category. It has ${productCount} product(s) associated with it.`);
        }
        const result = await database_1.pool
            .request()
            .input("id", id)
